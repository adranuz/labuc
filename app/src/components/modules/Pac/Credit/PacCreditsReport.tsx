...
import { PacCreditsReportTable } from './PacCreditsReportTable'
import { usePacStore } from '@/store/pac'
import { API_URL } from '@/utils/constants'

export function PacCreditsReport () {
  ...
  const [
    ...
    state.getCreditReportListLoading
  ])
  ...
  const handleDateStartChange = (value: Dayjs | null) => {
    ...
    if (value && endDate && customerId) {
      getCreditReportList(customerId, status, value.format('YYYY-MM-DD'), endDate.format('YYYY-MM-DD'))
    }
    if (value === null && endDate == null && customerId) {
      getCreditReportList(customerId, status)
    }
  }
  ...
  const handleDateEndChange = (value: Dayjs | null) => {
    ...
    if (startDate && value && customerId) {
      getCreditReportList(customerId, status, startDate.format('YYYY-MM-DD'), value.format('YYYY-MM-DD'))
    }
  ...
  const handleStatusChange = (event: SelectChangeEvent) => {
    ...
    setFilters(newFilters)
    if (customerId) {
      getCreditReportList(customerId, value, startDate?.format('YYYY-MM-DD'), endDate?.format('YYYY-MM-DD'))
    }
  }
  ...
  const downloadReport = (customerId?: string, status?: string, startDate?: string, endDate?: string) => {
    ...
    const url = new URL(`${API_URL}/pac/customers/${customerId}/credit/report/download`)
  ...
  useEffect(() => {
    ...
    getCreditReportList(customerId, status, startDate?.format('YYYY-MM-DD'), endDate?.format('YYYY-MM-DD'))
  }, [])
  ...
  return (
    <Container sx={{ mt: 4, mb: 4 }} maxWidth='xl'>
      <Paper variant='outlined' sx={{ width: '100%', mb: 2, pt: 1, position: 'relative' }}>
        <Toolbar
          title='Reporte de créditos PAC'
          pathRouteForBackButton={`/tool/pac/credits?customerId=${customerId}`}
        >
          <DatePicker
            value={startDate}
