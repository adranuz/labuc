...
class PacRepository {
    async getPacCustomerCreditStats({ id }) {
        ...
        if (!customer)
            return null;
        const dbName = customer === null || customer === void 0 ? void 0 : customer.dbName;
        if (!dbName)
            return null;
        let data = {
            generalCount: 0,
        ...
        try {
            const report = await prisma_client_1.default.$queryRaw `SELECT * FROM "PacCreditReport" WHERE client = ${dbName} ORDER BY created_at DESC LIMIT 1`;
            if (report.length > 0) {
                const { general_count: generalCount, status_count: statusCount, success_score: successScore, error_score: errorScore, created_at: createdAt } = report[0];
                const { ANUL: statusAnulCount, CANC: statusCancCount, DEFI: statusDefiCount, MORA: statusMoraCount, PEND: statusPendCount, } = statusCount;
                data = {
                    generalCount,
                    statusAnulCount,
                    statusCancCount,
    ...
    async getPacCustomerCreditReport({ id, status, startDate, endDate, perPage = 10, page = 0, q = '', pagination = true }) {
        var _a;
        const customer = await prisma_client_1.default.customer.findUnique({
            where: {
                id
        ...
        const dbUrl = dbName === 'telcelint_core'
            ...
            : (_a = process.env.PACS_DATABASE_URL_WITHOUT_DBNAME) === null || _a === void 0 ? void 0 : _a.concat(dbName);
    ...
    async getPacCustomerCreditReportFile({ id, status, startDate, endDate }) {
        var _a;
        const customer = await prisma_client_1.default.customer.findUnique({
            where: {
                id
    ...
    async getPacCustomerScoreReport({ id, status, startDate, endDate, perPage = 10, page = 0, q = '', pagination = true }) {
        var _a;
        const customer = await prisma_client_1.default.customer.findUnique({
            where: {
                id
        ...
        if (!dbName)
            return null;
        const mysql = require('mysql2/promise');
        const dbUrl = (_a = process.env.PACS_DATABASE_URL_WITHOUT_DBNAME) === null || _a === void 0 ? void 0 : _a.concat(dbName);
        const connection = await mysql.createConnection(dbUrl);
        const [data] = await connection.execute(`
    ...
    async getPacCustomerScoreReportFile({ id, status, startDate, endDate }) {
        var _a;
        const customer = await prisma_client_1.default.customer.findUnique({
            where: {
                id
