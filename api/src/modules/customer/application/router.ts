...
import {
  ...
  updateCustomerSchema, deleteCustomerSchema, getCustomerSchema, paginationFilterSchema, listCustomerSchema
} from './customer.schema';

export default (cradle: ICradle) => {
  const router = Router();

  router.get('/customers', cradle.customerMiddleware.validate(listCustomerSchema), cradle.customerController.listCustomers);
